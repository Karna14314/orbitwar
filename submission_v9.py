"""
Orbit Wars Agent — V9 SUPER-AGGRESSOR
Submit: kaggle competitions submit orbit-wars -f submission.py -m "V9 Super-Aggressor"

Core philosophy:
  - QUADRANT LOCALIZATION: Prioritizes nearby quadrant/corner static neutrals.
  - CO-ORBIT ring neighbors are captured immediately (+4000.0).
  - COOPERATIVE multi-launches and proactive reinforcement.
  - SUN evasion and precise ETA-garrison predictions.
  - COMET harvesting at minimum cost (exactly 2 ships).
"""
...
def score_target(src, tgt, eta, is_comet, step, needed, mine, planets, pid, state, already_sent=0):
    ...
    if dist < 25.0:
        score += (25.0 - dist) * 60.0

    # ── Quadrant Localization / Cluster-Building (V9 Core) ─────────────────
    if mine:
        ...
        if enemy_planets:
            ...
            min_dist_to_enemy = 999.0

        if min_dist_to_us < min_dist_to_enemy:
            # We are closer than the enemy (our home zone)
            score += (min_dist_to_enemy - min_dist_to_us) * 35.0
        else:
            # Enemy quadrant (skip or penalize early on)
            score -= (min_dist_to_us - min_dist_to_enemy) * 25.0

    # ── Early Static Neutral Corner Boost (V9 Core) ────────────────────────
    if tgt['owner'] == -1 and tgt['id'] not in state['moving']:
        # Strongly grab non-moving safe bases at corners in the first 80 turns
        score += max(0.0, (80.0 - step) * 75.0)

    # ── Neutral planet bonuses ──────────────────────────────────────────────
    if tgt['owner'] == -1:
        neutral_mult = max(1.0, 2.8 - (step / 400.0) * 1.8)
        score *= neutral_mult
        neutral_bonus = max(5, 250 - 0.6 * step - 25 * len(mine))
        score += neutral_bonus
...
def compute_moves(state, pid):
    ...
    for p in mine:
        ...
        safety_need = int(incoming_ships * 1.35 + 6)
