# HYPOTHESIS: Contested Zone Guard to avoid early expansion where enemy is closer
# ROUND: 2 | DATE: 2024-05-26
# BASED ON: champion.py
# CHANGELOG: Heavy penalty in score_target for contested zones if step < 150
import math
...
def score_target(src, tgt, eta, is_comet, step, needed, mine, planets, pid, state, already_sent=0):
    ...
    if enemy_planets:
        ...
        if min_dist_to_enemy < min_dist_to_us and step < 150:
            ev -= 10000.0
        elif min_dist_to_enemy < min_dist_to_us - 10.0 and step < 250:
