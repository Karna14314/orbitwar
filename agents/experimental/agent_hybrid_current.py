# HYPOTHESIS: Hybrid tuning combining speed buffer 1.38 and anchor bonus changes
# ROUND: 1 | DATE: 2024-05-31
# BASED ON: champion.py
# CHANGELOG: send buffer 1.35->1.38, anchor_bonus base 40->45
import math
...
def score_target(src, tgt, eta, is_comet, step, needed, mine, planets, pid, state, already_sent=0):
    ...
    if not is_moving:
        anchor_bonus = max(0.0, 45.0 - dist) * 12.0
        ev += anchor_bonus
            ...
            for tgt in targets:
                ...
                if is_comet:
                    ...
                    for _ in range(3):
                        ...
                        send = min(int(max_send), max(int(needed * 1.38), needed + 3))
