# HYPOTHESIS: Wave Expansion Structural Change
# ROUND: 1 | DATE: 2024-05-26
# BASED ON: champion.py
# CHANGELOG: Prioritize closer expansion targets heavily
import math
...
def score_target(src, tgt, eta, is_comet, step, needed, mine, planets, pid, state, already_sent=0):
    ...
    if tgt['owner'] == -1:
        # CHANGELOG: Increase base value of neutral expansion if distance is short
        distance_discount = 1.0 / (dist + 1.0)
        neutral_mult = max(1.0, 2.8 - (step / 400.0) * 1.8)
        ev *= neutral_mult
        ev += max(5.0, 250.0 - 0.6 * step - 25.0 * len(mine)) * distance_discount * 10
