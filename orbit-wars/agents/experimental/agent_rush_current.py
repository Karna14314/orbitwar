# HYPOTHESIS: Early rush on the nearest neutral/lowest-health post. Modifies EV to heavily prioritize targets with low garrison size.
# DATE: 2024-05-20
# BASED ON: agent_heuristic_current.py
# CHANGELOG: Modified EV calculation to penalize high garrison size targets, incentivizing quick capture of low-health/neutral outposts.
...
def heuristic_moves(state, pid, exclude_targets=None):
    """
    Rush heuristic. Heavily prioritizes targets with low garrison size, particularly early game.
    """
    ...
    for src in mine_sorted:
        ...
        for tgt in targets:
            ...
            # Massive priority to early game neutral targets, and specifically those with low health
            if tgt['owner'] == -1 and state['step'] < 200:
                ev *= 2.5 # Boosted neutral focus
                # Rush modifier: highly penalize large garrisons early on to prioritize fast/cheap caps
                ev -= tgt['ships'] * 20.0
            else:
                # Penalize garrisons universally to prefer soft targets
                ev -= tgt['ships'] * 10.0
